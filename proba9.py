...
def find_coords(text, postal_code=False):
    try:
        ...
        if postal_code:
            try:
                ...
                adress += '\nПочтовый индекс: ' + str(code)
            except Exception as e:
                print(e)
                print('Нет почтового индекса')
        return coords.split(' '), adress
    except Exception as e:
        print(e)
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.pushButton.setGeometry(QtCore.QRect(610, 160, 141, 41))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(9)
        ...
        self.pushButton_2.setGeometry(QtCore.QRect(610, 210, 141, 41))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(9)
        ...
        self.pushButton_3.setGeometry(QtCore.QRect(610, 260, 141, 41))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(9)
        ...
        self.pushButton_4.setGeometry(QtCore.QRect(750, 40, 141, 41))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(9)
        ...
        self.pushButton_5.setGeometry(QtCore.QRect(610, 90, 281, 41))
        font = QtGui.QFont()
        font.setFamily("Arial")
        font.setPointSize(9)
        ...
        self.textBrowser.setGeometry(QtCore.QRect(0, 470, 601, 61))
        font = QtGui.QFont()
        font.setFamily("Arial")
        self.textBrowser.setFont(font)
        self.textBrowser.setObjectName("textBrowser")
        self.checkBox = QtWidgets.QCheckBox(self.centralwidget)
        self.checkBox.setGeometry(QtCore.QRect(620, 480, 271, 31))
        font = QtGui.QFont()
        font.setFamily("Arial")
        self.checkBox.setFont(font)
        self.checkBox.setStyleSheet("background-color: rgb(212, 245, 255);")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pushButton_5.setText(_translate("MainWindow", "Cброс поискового результата"))
...
class Map_window(QMainWindow, Ui_MainWindow):
    def __init__(self):
        try:
            ...
            self.checkBox.stateChanged.connect(self.show_postal)
            self.show()
        except Exception as e:
            print(e)
    ...
    def find_func(self):
        ...
        coords, adress= find_coords(text,self.get_postal)
        if coords:
            self.x, self.y = float(coords[0]), float(coords[1])
            self.pt = ','.join(coords)
            self.map = new_request(self.x, self.y, self.delta_x, self.delta_y, self.layer, self.pt)
            self.label_2.setPixmap(QPixmap(self.map))
            self.textBrowser.setText(adress)
    ...
    def show_postal(self, state):
        if state == Qt.Checked:
            self.get_postal = True
        else:
            self.get_postal = False
        if self.pt:
            self.find_func()
